...
public class SimulationController {
    ...
    public ResponseEntity<ApiResponse<BaseSimulationResponseDto>> createSimulation(
            @Valid @RequestBody BaseCreateSimulationRequestDto requestDto,
            @AuthenticationPrincipal User user) {
        BaseSimulationResponseDto response = simulationService.saveSimulation(requestDto, user, requestDto.getGoalIds());
    ...
    public ResponseEntity<ApiResponse<List<SimulationSummaryDto>>> getSimulations(@AuthenticationPrincipal User user) {
...
//    public ResponseEntity<BaseSimulationResponseDto> getSimulation(@AuthenticationPrincipal User user,@PathVariable Long simulationId) {
//        BaseSimulationResponseDto simulation = simulationService.findSimulationById(useroi, simulationId);
//        return ResponseEntity.ok(simulation);
//    }

    //시뮬레이션 softdelete삭제 로직
    ...
    public ResponseEntity<ApiResponse<List<DeletedSimulationResponseDto>>> getSoftSimulation(@AuthenticationPrincipal User user) {
        List<DeletedSimulationResponseDto> simulations = simulationService.findAllSoftDeletedSimulations(user.getId());
...
//    //시뮬레이션 소프트삭제 로직
//    @Operation(summary = "시뮬레이션 소프트 삭제", description = "시뮬레이션을 소프트 딜리트 방식으로 삭제합니다.", security = @SecurityRequirement(name = "bearerAuth"))
//    @DeleteMapping("/simulations/{simulationId}/soft-delete")
//    public ResponseEntity<ApiResponse<DeletedSimulationResponseDto>> softDeleteSimulation(
//            @AuthenticationPrincipal User user,
//            @PathVariable Long simulationId) {
//        DeletedSimulationResponseDto simulation = simulationService.softDeleteSimulation(user.getId(), simulationId);
...
//    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteSimulation(
            @AuthenticationPrincipal User user,
            @PathVariable Long simulationId) {
        simulationService.deleteSimulation(user.getId(), simulationId); // 실제 삭제 로직 분리 필요
