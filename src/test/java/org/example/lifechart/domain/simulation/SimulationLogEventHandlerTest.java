...
public class SimulationLogEventHandlerTest {
    ...
    void 시뮬레이션로그_저장_이벤트_정상_발행() {
        ...
        GoalRetirement goalRetirement = GoalRetirement.builder()
                .goal(retirementGoal)
                .expectedDeathDate(LocalDate.of(2080, 1, 1))
                .build();

        given(goalRetirementRepository.findByGoalId(retirementGoal.getId()))
                .willReturn(Optional.of(goalRetirement));

        given(goalRepository.findAllWithUserByIdAndUserId(List.of(1L, 2L), 1L))
                .willReturn(List.of(goal, retirementGoal));
        ...
        simulationService.saveSimulation(dto, user.getId(), List.of(1L, 2L));
    ...
    void 목표수정_시뮬레이션수정_정상발행() {
        ...
        given(goalRepository.findByIdAndUserIdAndStatus(goal.getId(), user.getId(), Status.ACTIVE))
        ...
        GoalRetirement retirementDetail = GoalRetirement.builder()
                .goal(retirementGoal)
                .expectedDeathDate(LocalDate.of(2080, 1, 1))
                .build();

        given(goalRetirementRepository.findByGoalId(retirementGoal.getId()))
                .willReturn(Optional.of(retirementDetail));
        ...
        SimulationGoal housingSimGoal = SimulationGoal.builder()
                .goal(goal)
        ...
        // 시뮬레이션 골 조회
        given(simulationGoalRepository.findAllByGoalIdAndSimulationUserIdAndActiveTrue(goal.getId(), user.getId()))
                .willReturn(List.of(housingSimGoal));

        // 시뮬레이션에 연결된 활성 골 리스트 반환 (은퇴 포함)
        given(simulationGoalRepository.findActiveGoalsBySimulationId(simulation.getId()))
                .willReturn(List.of(goal, retirementGoal));

        SimulationResults mockResults = SimulationResults.builder()
                .requiredAmount(8_000_000L)
                .estimatedAchieveMonth("2025-06")
                .currentAchievementRate(10.0f)
                .monthlyAchievements(List.of())
                .monthlyAssets(List.of())
                .build();
    ...
    void 시뮬레이션수정_시_이벤트_정상_발행() {
        ...
        given(goalRepository.findAllWithUserByIdAndUserId(List.of(1L, 2L), user.getId()))
                .willReturn(List.of(goal, retirementGoal));
        ...
        UpdateSimulationRequestDto dto1 = new UpdateSimulationRequestDto(
                "5년 뒤 내 집 마련",
                LocalDate.of(2025, 6, 16),
                1_000_000L,
                3_000_000L,
        ...
        simulationService.updateSimulationSettings(user.getId(), simulation.getId(), dto1);
